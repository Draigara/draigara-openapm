...
if (-not $manifest.Contains('ref: v0.1.0-preview.1')) { $errors.Add('apm.yml is missing the reviewed Forge plugin tag') }
...
foreach ($relative in @('.claude-plugin/marketplace.json', '.agents/plugins/marketplace.json')) {
    ...
    try {
        $generated = Get-Content -Raw -LiteralPath $path | ConvertFrom-Json
        $forge = @($generated.plugins) | Where-Object { $_.name -eq 'draigara-forge' } | Select-Object -First 1
        if ($forge.source.ref -ne 'v0.1.0-preview.1' -or $forge.source.sha -ne '8ba52fa611640f783a416724eb006c675e4d9d40') {
            $errors.Add("Generated marketplace does not pin Forge plugin preview.1 at the reviewed tag object: $relative")
        }
    }
